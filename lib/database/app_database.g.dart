...
class _$AppDao extends AppDao {
      ...
      : _queryAdapter = QueryAdapter(database, changeListener),
        ...
        _alertEntityInsertionAdapter = InsertionAdapter(
            ...
            (AlertEntity item) => <String, dynamic>{
                  ...
                  'limitDirection': _marketDirectionConverter.encode(item.limitDirection),
  ...
  Future<List<AlertEntity>> findAllAlerts() async {
    return _queryAdapter.queryList('SELECT * FROM alerts',
        mapper: (Map<String, dynamic> row) => AlertEntity(
            ...
            limitDirection: _marketDirectionConverter.decode(row['limitDirection'] as int),
  ...
  Stream<List<AlertEntity>> findActiveAlertsAsStream() {
    return _queryAdapter.queryListStream('SELECT * FROM alerts WHERE triggerState=1',
        queryableName: 'alerts',
        isView: false,
        mapper: (Map<String, dynamic> row) => AlertEntity(
            id: row['id'] as int,
  ...
  Future<int> insertTicker(TickerEntity ticker) {
    return _tickerEntityInsertionAdapter.insertAndReturnId(ticker, OnConflictStrategy.replace);
  }

  @override
  Future<List<int>> insertTickers(List<TickerEntity> tickers) {
    return _tickerEntityInsertionAdapter.insertListAndReturnIds(tickers, OnConflictStrategy.replace);
  }

  @override
  Future<int> insertTickerWatch(TickerWatchEntity ticker) {
    return _tickerWatchEntityInsertionAdapter.insertAndReturnId(ticker, OnConflictStrategy.abort);
  }

  @override
  Future<int> insertAlert(AlertEntity alert) {
    return _alertEntityInsertionAdapter.insertAndReturnId(alert, OnConflictStrategy.abort);
  }

  @override
  Future<int> updateTicker(TickerEntity ticker) {
    return _tickerEntityUpdateAdapter.updateAndReturnChangedRows(ticker, OnConflictStrategy.abort);
  }

  @override
  Future<int> updateAlert(AlertEntity alert) {
    return _alertEntityUpdateAdapter.updateAndReturnChangedRows(alert, OnConflictStrategy.abort);
  }
